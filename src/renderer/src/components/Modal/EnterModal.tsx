...
const EnterModal = () => {
  ...
  const fetchDropdownFields = async () => {
    const contractorResponse = await axios.get(`${API_URL}/getOrganizations?role=ROLE_TRANSPORTER`)
    // console.log()
    setContractors((state) => [...state, ...contractorResponse.data.items])

    const cargoCategoriesResponse = await axios.get(`${API_URL}/getCargoCategories`)
    // console.log('car', JSON.stringify(cargoCategoriesResponse.data.items))
    setCargoCategories((state) => [...state, ...cargoCategoriesResponse.data.items])
        ...
        <div className="container">
          <div className="modal__header d-flex justify-content-center">
            <h2 className="modal__title text-center">
              Взвешивание брутто:{' '}
              <span style={{ opacity: isUpdating ? '0' : '1' }}>{terminalWeight} кг</span>
            </h2>
            <svg
          ...
          <form className="modal__form mt-5" onSubmit={submitHandler}>
            ...
            <div className="d-flex justify-content-between mt-5">
              <Button label="Отклонить" variant="danger" onClick={closeModal} type="button" />
              <Button
